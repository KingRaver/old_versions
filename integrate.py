...
class ConfigurationManager:
    ...
    def __init__(self):
        ...
        self.CONFIDENCE_THRESHOLDS = {
            'BTC': {'base': 75, 'volatile': 80, 'stable': 70},
            'ETH': {'base': 73, 'volatile': 78, 'stable': 68},
            'SOL': {'base': 68, 'volatile': 75, 'stable': 65},
            'XRP': {'base': 70, 'volatile': 77, 'stable': 67},
            'BNB': {'base': 73, 'volatile': 78, 'stable': 68},
            'AVAX': {'base': 70, 'volatile': 77, 'stable': 67}
        }
...
class PredictionManager:
    ...
    def _normalize_prediction_format(self, prediction_dict: Dict[str, Any]) -> Dict[str, Any]:
        """
        Convert prediction engine dictionary format to trading bot expected format
        Extracts nested fields and creates a flat structure for compatibility
        """
        try:
            # Start with the original dict
            normalized = prediction_dict.copy()
        
            # Extract nested volatility_score from risk_assessment
            risk_assessment = prediction_dict.get('risk_assessment', {})
            normalized['volatility_score'] = risk_assessment.get('volatility_score', 50.0)
            normalized['prediction_risk_score'] = risk_assessment.get('prediction_risk_score', 50.0)
        
            # Extract nested market_condition from market_analysis
            market_analysis = prediction_dict.get('market_analysis', {})
            normalized['market_condition'] = market_analysis.get('market_condition', 'UNKNOWN')
        
            # Extract data_quality_score if it exists
            metadata = prediction_dict.get('metadata', {})
            if 'data_quality_score' not in normalized:
                normalized['data_quality_score'] = metadata.get('quality_checks_passed', True) * 100.0
        
            # Add derived direction field based on expected_return_pct
            expected_return = normalized.get('expected_return_pct', 0)
            ...
            # Ensure confidence is at top level (it should be, but double-check)
            if 'confidence' not in normalized and 'prediction_confidence' in prediction_dict:
                normalized['confidence'] = prediction_dict['prediction_confidence']
            
            return normalized
        
        except Exception as e:
            print(f"⚠️ Prediction normalization failed: {e}")
            return prediction_dict  # Return original if normalization fails
    ...
    async def generate_trading_prediction(self, token: str, force_refresh: bool = False) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if prediction_result:
                ...
                prediction_result = self._normalize_prediction_format(prediction_result)
    ...
    async def batch_generate_predictions(self, tokens: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        Generate predictions for multiple tokens using your advanced system
        """
        if tokens is None:
        ...
        for token in tokens:
            try:
                prediction = await self.generate_trading_prediction(token)
                if prediction:
                    predictions[token] = prediction
            except Exception as e:
                print(f"❌ Batch prediction failed for {token}: {e}")
    ...
    async def _get_market_data(self, token: str) -> Optional[Dict[str, Any]]:
        """
        Get market data for prediction - uses your existing system's approach
        """
        try:
            # Get current price
            ...
            # Create market data structure compatible with your prediction engine
            market_data = {
                'token': token,
            ...
            # Add any additional market data your system needs
            # This can be expanded based on what your prediction_engine.py expects
            
            return market_data
            
        except Exception as e:
            print(f"⚠️ Failed to get market data for {token}: {e}")
            return None
    
    async def _get_current_price(self, token: str) -> Optional[float]:
        """Get current price for token using available price sources"""
        try:
            # Try to use multi-chain manager first
            if MULTI_CHAIN_AVAILABLE and self.multi_chain_manager:
                price = await self.multi_chain_manager._get_token_price(token)
                if price:
                    return price
            ...
            return fallback_prices.get(token, 100.0)
            
        except Exception as e:
            print(f"⚠️ Price fetch failed for {token}: {e}")
            return None
...
class TradeExecutionManager:
    ...
    async def _monitor_position(self, monitoring_task: MonitoringTask):
        ...
        try:
            ...
            if MULTI_CHAIN_AVAILABLE:
                current_price = await self.multi_chain_manager._get_token_price(monitoring_task.token)
            else:
...
class TradingBot:
    ...
    async def _execute_best_opportunities(self, opportunities: List[Dict[str, Any]]):
        ...
        try:
            ...
            for opportunity in opportunities[:max_executions]:
                try:
                    ...
                    execution_id = await self.execution_manager.execute_prediction_trade(
                        {
                            ...
                            'market_condition': prediction.market_condition.value if hasattr(prediction.market_condition, 'value') else 'UNKNOWN'
                        }
                    )
                    ...
                    if execution_id:
                        ...
                        self.performance.predictions_generated += 1
                    ...
                    print(f"❌ Individual trade execution failed: {e}")
                    continue
            ...
            print(f"❌ Trade execution batch failed: {e}")
